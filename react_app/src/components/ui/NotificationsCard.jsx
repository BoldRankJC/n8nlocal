...
const NotificationsCard = ({ user, onUnreadChange }) => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const res = await fetch(`https://Boostedapi.vercel.app/api/noti/${user}`);
  ...
  const handleDeleteNotifications = async () => {
    try {
      ...
      const res = await fetch(`https://Boostedapi.vercel.app/api/noti/${mail}`, {
        method: "DELETE",
        headers: {
  ...
  const handleDeleteNotification = async (id) => {
    try {
      ...
      const res = await fetch(`https://Boostedapi.vercel.app/api/noti/${mail}/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const res = await fetch(`https://Boostedapi.vercel.app/api/noti/${mail}/leido-todas`, {
        method: "PUT",
    ...
    <div className="bg-card rounded-xl m-4 shadow-brand border border-border">
      <div className="space-y-4 max-h-96 overflow-y-auto">
        {notifications && notifications.length > 0 ? (
          ...
          <div
              key="0"
              className={`border rounded-lg p-4 m-2 transition-brand cursor-pointer hover:shadow-brand-hover border-border bg-card`}
            >
              <div className="flex items-start space-x-4">
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between">
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground mb-2">
                        Aqui aparecerán sus notificaciones próximamente
                      </p>
                      <div className="flex items-center space-x-3">
                        <span className="text-xs text-muted-foreground">
                          0 m
