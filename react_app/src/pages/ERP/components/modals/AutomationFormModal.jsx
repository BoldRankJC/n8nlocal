...
export const AutomationFormModal = ({
    isOpen,
    onClose,
    onSave,
    initialData,
    ...
    const [autoData, setAutoData] = useState({ name: '', trigger: '', action: '' });

    useEffect(() => {
        if (initialData) {
            setAutoData(initialData);
        } else {
            setAutoData({ name: '', trigger: '', action: '' });
        }
    }, [initialData, isOpen]);
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-lg w-full">
                <div className="p-6 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{isEditing ? 'Editar Regla' : 'Nueva Automatización'}</h2>
